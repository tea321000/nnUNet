...
import numpy as np
import torch
from nnunet.network_architecture.custom_modules.flare_conv_blocks import BasicResidualBlock, ResidualLayer, StackedConvLayers
...
class FabiansUNet_flare(SegmentationNetwork):
    """
    ...
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage_encoder, feat_map_mul_on_downscale,
        ...
        if props_decoder is None:
            props_decoder = props
        self.decoder = CutLossPlainConvUNetDecoder(self.encoder, num_classes, num_blocks_per_stage_decoder,
                                                   props_decoder,
...
def find_3d_configuration():
    ...
    print(
        FabiansUNet.compute_approx_vram_consumption(patch_size, initial_num_features, max_num_features, num_modalities,
                                                    num_classes, pool_op_kernel_sizes,
                                                    blocks_per_stage_encoder[:len(conv_op_kernel_sizes)],
                                                    blocks_per_stage_decoder[:len(conv_op_kernel_sizes) - 1], 2,
                                                    batch_size))
